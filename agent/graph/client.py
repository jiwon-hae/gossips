...
class GraphitiClient:
    def __init__(
        ...
        """
        # Neo4j configuration
        self.neo4j_uri = neo4j_uri or env.get(
            "NEO4J_URI", "bolt://localhost:7687")
        self.neo4j_user = neo4j_user or env.get("NEO4J_USER", "neo4j")
        self.neo4j_password = neo4j_password or env.get("NEO4J_PASSWORD")

        if not self.neo4j_password:
            raise ValueError("NEO4J_PASSWORD environment variable not set")

        # LLM configuration
        self.llm_base_url = config.get(
            "LLM_BASE_URL", "https://api.openai.com/v1")
        self.llm_api_key = env.get("LLM_API_KEY")
        self.llm_choice = config.get("LLM_CHOICE", "gpt-4.1-mini")
        ...
        self.embedding_dimensions = int(config.get("VECTOR_DIMENSION", "1536"))
    ...
    async def get_related_entities(
        self,
        entity_name: str,
        relationsihp_types: Optional[List[str]] = None,
        depth: int = 1
    ) -> Dict[str, Any]:
        """
        ...
        query = prompt.render('relationship.j2', entity=entity_name)
        results = await self.graphiti.search(query)
    ...
    async def get_entity_timeline(
        ...
        query = prompt.render('timeline.j2', entity=entity_name)
        results = await self.graphiti.search(query)
    ...
    async def clear_graph(self):
        ...
        try:
            ...
            logger.warning(
                "Reinitialized Graphiti client (fresh indices created)")
